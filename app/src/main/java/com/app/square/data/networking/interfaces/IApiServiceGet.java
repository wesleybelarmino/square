...
public interface IApiServiceGet {
    ...
    @GET("3/movie/{id}/videos") Observable<TrailersResult> trailersByMovie(@Path("id") int movie_id,
        @Query("api_key") String api_key, @Query("language") String language);

    @GET("3/movie/{id}/reviews") Observable<ReviewsResult> reviewsByMovie(@Path("id") int movie_id,
        @Query("api_key") String api_key, @Query("language") String language,
        @Query("page") String page);
}